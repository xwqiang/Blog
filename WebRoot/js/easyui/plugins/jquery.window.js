...
function openWinW(URL,winName,winWidth,winHeight,isMax,isMin){
   if(winName == null) {
	...
	var boardDiv = "<div id=\"win_div_"+winName+"\" class=\"easyui-window\" title=\"My Window\" style=\"width:"+winWidth+";height:"+winHeight+";padding:5px;background: #fafafa;\"><\/div>"; 
	$(document.body).append(boardDiv);
	var boardIframe = "<iframe id=\"win_iframe_"+winName+"\" src=\"\" style=\"width:99%;height:98.5%\" scrolling=\"no\"></iframe>";
	...
	$("#win_iframe_"+winName).attr("src", URL); 
}
		function judgeSelect(judgeObj){
			if(judgeObj==null){
				$.messager.alert("警告","请给judgeSelect方法赋值对象！","info");
				return null;
			};
			var selectedRow = $("#"+judgeObj).datagrid("getSelected");
			if(selectedRow==null){
